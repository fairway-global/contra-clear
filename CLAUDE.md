...
- **Token icons:** SVG images from `https://raw.githubusercontent.com/fairway-global/otc-stablecoins/main/metadata/{SYMBOL}.svg`
- **TokenIcon component:** `src/components/ui/TokenIcon.tsx` — reusable, fallback to letter circle

---

## Settlement Flow (Atomic Swap)

### How Settlement Works
After quote acceptance, RFQ goes directly to `ReadyToSettle`. Settlement is an **atomic swap** within the Contra channel — a single transaction containing both transfers that requires both parties to sign before it can execute. Either both transfers happen or neither does.

1. **Auto-register wallets:** When either party opens the RFQ detail page, their wallet is auto-registered via `POST /settlement/register-wallet` (uses `useRef` to prevent infinite loops)
2. **Build atomic swap tx:** Once both wallets registered, `POST /settlement/build` creates ONE unsigned V0 VersionedTransaction via `swap.ts` containing both transfer instructions:
   - Instruction 0: Originator's sell tokens → LP (within Contra channel)
   - Instruction 1: LP's buy tokens → Originator (within Contra channel)
   - Both parties are required signers (2 signature slots)
3. **Partial sign:** Each party clicks "Sign Settlement" → Phantom partially signs the atomic tx (only their slot) → 64-byte signature sent to backend as base64
4. **Assemble & execute:** When both partial signatures are collected, backend inserts them into the correct slots of the VersionedTransaction and submits the fully-signed tx to the Contra gateway. Confirmation via `getSignatureStatuses` polling.
5. **Complete:** `completeAtomicSettlement()` marks escrows Released, RFQ Settled

### Settlement Database Fields (on `otc_rfqs`)
- `originator_wallet` — Originator's Solana public key (auto-registered)
- `provider_wallet` — LP's Solana public key (auto-registered)
- `atomic_swap_tx` — Unsigned atomic swap transaction (base64 V0 VersionedTransaction)
- `atomic_swap_signers` — JSON array of required signer pubkeys `["partyA", "partyB"]`
- `settlement_leg_a_sig` — Originator's 64-byte partial signature (base64)
- `settlement_leg_b_sig` — LP's 64-byte partial signature (base64)
- `settlement_leg_a_tx`, `settlement_leg_b_tx` — Legacy fields (unused in atomic flow)

### Settlement API Endpoints
| Method | Path | Description |
|--------|------|-------------|
| POST | `/settlement/register-wallet` | Register wallet address for settlement |
| POST | `/settlement/build` | Build atomic swap tx (validates Contra balances) |
| GET | `/settlement/:rfqId` | Get settlement info |
| POST | `/settlement/submit-leg` | Submit partial signature; auto-executes when both collected |

### Frontend Settlement Flow (`sendToContra.ts`)
- `partialSignForContra()` — Signs the atomic tx with the connected wallet but does NOT submit. Extracts and returns the 64-byte signature for the wallet's signer slot.
- `signAndSendToContra()` — Legacy: signs + submits (used for non-atomic operations like deposits)

### Known Settlement Issues & Constraints

1. **Receiver ATA must exist on channel:** The Contra channel doesn't support `createAssociatedTokenAccountInstruction` because users don't have SOL on the channel. Only the operator creates ATAs during the deposit flow. **Both parties must have deposited each token at least once** before a swap involving that token can work. The proper fix is to have the operator pre-create ATAs, but this requires operator keypair access on the channel.

2. **Gateway returns signature even on failed txs:** The Contra gateway's `sendTransaction` returns a tx hash even if the transaction fails internally. `getTransaction` is not supported. Use `getSignatureStatuses` to verify execution and detect errors.

3. **Blockhash expiry:** The atomic swap tx is built with a blockhash from the gateway. If there's a long delay between building and both parties signing, the blockhash may expire. The tx should be rebuilt if signing takes too long.

4. **Token Program:** All tokens are Token-2022 on Solana mainnet, but the Contra channel uses standard `TOKEN_PROGRAM_ID` for ATAs and transfers. The `swap.ts` correctly uses `TOKEN_PROGRAM_ID` for channel operations. The `WithdrawPanel` uses `TOKEN_2022_PROGRAM_ID` for detecting release transactions on mainnet.

### Test Script
`scripts/test-atomic-swap.ts` — End-to-end test that creates two wallets, mints tokens, deposits to Contra, builds an atomic swap, both wallets sign, submits, and verifies balances. Also verifies that single-signature submissions don't execute (atomicity). Run with: `cd otc-server && npx tsx ../scripts/test-atomic-swap.ts`
...
- `POST /mint` — Mint up to 1000 tokens to any wallet
- Mint authority loaded from `FAUCET_MINT_AUTHORITY_SECRET` env var (JSON byte array) or file at `SAS_PAYER_PATH`
- All 11 tokens are Token-2022 with 6 decimals
- Auto-creates receiver ATA if needed

### Frontend (`/faucet`)
- Token grid with SVG icons
- Amount input (1-1000)
- Phantom wallet auto-detected
- Success toast with Solscan link
- Yellow "Faucet" button in header (next to wallet)

### Available Tokens
USDC, USDT, EURC, PYUSD, USDG, USX, CHF, GBP, JPY, SGD, AED

---

## KYC Bypass (Test Accounts)
Backend hardcodes bypass for these emails in the `/kyc/status` endpoint:
- `lp@contraotc.dev`
- `rfq@contraotc.dev`
- `admin@contraotc.dev`

These always return `kycStatus: 'verified'` regardless of wallet.
...
### Production URL
- `contraclear.fairway.global` (allowed in vite.config.ts)

### Vite Config
- `allowedHosts`: `['super-ghost-intimate.ngrok-free.app', 'contraclear.fairway.global']`
- Proxy: `/api` and `/health` → backend

### Key Deployment Notes
- `.env` files are gitignored — secrets must be set on the server
- `FAUCET_MINT_AUTHORITY_SECRET` env var avoids needing to upload keypair files
- Delete `otc.db` when schema changes are made (auto-recreated on restart)
- All 3 services must be running: backend (3001), frontend (5173), webhook (8085)
- WebSocket on port 3002 provides real-time updates with exponential backoff reconnect
